four51.app.factory("Product", [
	"$resource",
	"$451",
	"Security",
	"User",
	function($resource, $451, Security, User) {
		var variantCache = [],
			productCache = [],
			criteriaCache;
		function _then(fn, data, count) {
			if (angular.isFunction(fn)) fn(data, count);
		}
		...
		var _get = function(param, success, page, pagesize, searchTerm) {
			...
			var product = $resource($451.api("Products/:interopID"), {
				interopID: "@ID"
			})
				.get({
					interopID: param,
		...
		var _search = function(
			...
			if (
				typeof productCache[(criteria.Page - 1) * criteria.PageSize] ==
					"object" &&
				typeof productCache[criteria.Page * criteria.PageSize - 1] == "object"
			) {
				_then(success, productCache, productCache.length);
			} else {
				$resource($451.api("Products"))
					.get(criteria)
					.$promise.then(function(products) {
						...
						for (var i = 0; i <= products.Count - 1; i++) {
							...
							productCache[i] =
								products.List[
									i - ((criteria.Page || 1) - 1) * (criteria.PageSize || 100)
								] || i;
						}
