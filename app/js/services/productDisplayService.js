	...
	function($sce, $451, Variant, Product) {
		function calcTotal(lineItem) {
			...
			angular.forEach(ps.PriceBreaks, function(pb) {
				if (lineItem.Quantity >= pb.Quantity) priceBreak = pb; //assumes they will be in order of smallest to largest
			});
			if (!priceBreak) {
			...
			var debugLineTotal =
				"line total debug:\rquantity:" +
				lineItem.Quantity +
				" & " +
				"amount added per quantity:" +
				amountPerQty +
				" & " +
				"fixed ammount per line added:" +
				fixedAddPerLine +
				" & " +
				"percentage added to qty*unitprice:" +
				percentagePerLine +
		...
		function productViewScope(scope) {
			...
			scope.setAddToOrderErrors = function() {
				...
				if (scope.allowAddFromVariantList) {
					...
					angular.forEach(scope.variantLineItems, function(item) {
						if (item.Quantity > 0) haveQty = true;
					...
					if (
						...
						(scope.variantLineItems &&
							!Object.keys(scope.variantLineItems).length)
					)
						newErrorList.push("Please create a variant.");
					else if (!haveQty && !haveQtyError)
						newErrorList.push("Please select a quantity");
				} else if (!scope.LineItem.Quantity && !scope.LineItem.qtyError)
					//if there's a qty error, just use that. in this case, there's no qty error because it hasn't been validated yet.
					newErrorList.push("Please select a quantity.");

				if (scope.LineItem.qtyError) newErrorList.push(scope.LineItem.qtyError);
				...
				if (spec.DefinesVariant) {
					...
					if (hasAllVarDefiningSpecs) {
						...
						Variant.get(
							...
							function(data) {
								if (!data.IsDefaultVariant) scope.LineItem.Variant = data;
			...
			scope.inventoryDisplay = function(product, variant) {
				...
				if (qa) return qa.QuantityAvailable > 0 ? qa.QuantityAvailable : 0;
				else return null;
			};
			if (scope.LineItem.Variant) {
				//scope.LineItem.Variant = variant;
				scope.StaticSpecGroups =
					scope.LineItem.Variant.StaticSpecGroups ||
					scope.LineItem.Product.StaticSpecGroups;
			} else {
		...
		function newLineItemScope(scope) {
			...
			if (!scope.LineItem.Specs) {
				//it's possible we're reloading this due to changing a variant and we don't want to leave the spec values behind
				scope.LineItem.Specs = {};
			...
			function determinePriceSchedule() {
				...
				if (scope.currentOrder && scope.currentOrder.Type == "Replenishment") {
					if (
						scope.LineItem.Variant &&
						scope.LineItem.Variant.ReplenishmentPriceSchedule
					)
					...
					if (
						scope.LineItem.Variant &&
						scope.LineItem.Variant.StandardPriceSchedule
					)
					...
					return (
						scope.LineItem.Product.ReplenishmentPriceSchedule ||
						scope.LineItem.Product.StandardPriceSchedule
					);
				} else {
					if (scope.LineItem.Variant && !scope.LineItem.Product.IsVBOSS)
						return (
			...
			if (scope.LineItem.Variant) {
				...
				!scope.LineItem.Product.IsVariantLevelInventory &&
				(scope.currentOrder && scope.currentOrder.Type == "Replenishment")
			) {
			...
			function allowAddToOrder() {
				return (
					...
					(scope.allowAddFromVariantList ||
						(scope.LineItem.Variant ||
							(scope.LineItem.Product.VariantCount == 0 &&
								scope.LineItem.Product.Type != "VariableText")))
				);
			}
			...
			function canAddToOrderType(type) {
				...
				if (
					scope.currentOrder &&
					scope.currentOrder.Type == "Replenishment" &&
					type == "Replenishment" &&
					!scope.LineItem.Product.IsVariantLevelInventory &&
					(scope.LineItem.PriceSchedule &&
						scope.LineItem.PriceSchedule.OrderType == "Replenishment")
				) {
					scope.allowAddToOrder = scope.user.Permissions.contains(
						type + "Order"
					); // even have to reset this
					return true;
				}
			...
			scope.LineItem.Product.DisplayInventory =
				scope.LineItem.Product.DisplayInventory &&
				(!scope.currentOrder ||
					(scope.currentOrder && scope.currentOrder.Type == "Standard"));
		...
		function staticSpecSPAConfig(product, specName) {
			if (!product.StaticSpecGroups) return null;
			if (!product.StaticSpecGroups.SPAProductConfig) return null;
			if (!product.StaticSpecGroups.SPAProductConfig.Specs[specName])
				return null;
			return (
				product.StaticSpecGroups.SPAProductConfig.Specs[specName].Value ||
				escapeNull
			);
		}
					...
					if (variantInteropID) {
						...
						if (p.Type == "Static" && p.IsVBOSS) {
							...
							angular.forEach(p.Specs, function(s) {
								if (s.DefinesVariant) {
									if (s.Value) options.push(s.SelectedOptionID);
									else hasAllDefinesAsVariant = false;
								}
							});
