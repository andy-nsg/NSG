four51.app.factory("Order", [
	...
	"Error",
	"User",
	function($resource, $rootScope, $451, Security, Error, User) {
		var _multipleShip = false;
		function _then(fn, data, broadcast) {
			if (angular.isFunction(fn)) fn(data);
			if (!broadcast) $rootScope.$broadcast("event:orderUpdate", data);
		}
		...
		function _extend(order, user) {
			...
			angular.forEach(order.LineItems, function(item) {
				...
				angular.forEach(item.Specs, function(spec) {
					if (
						spec.ControlType == "File" &&
						spec.File &&
						spec.File.Url.indexOf("auth") == -1
					)
			...
			order.IsMultipleShip = function() {
				...
				angular.forEach(order.LineItems, function(li, i) {
					...
					multi =
						i > 0
							? li.ShipAddressID != order.LineItems[i - 1].ShipAddressID ||
								li.ShipperID != order.LineItems[i - 1].ShipperID
							: false;
				});
				return multi;
			};
			...
			angular.forEach(order.LineItems, function(li) {
				if (li.IsKitParent) {
					...
					while (current) {
						if (!current.IsConfigured) li.KitIsInvalid = true;
						current = current.NextKitLineItem;
					}
				}
			});

			order.BillingEnabled =
				order.Total > 0 ||
				(order.Total == 0 && user.Company.BillZeroPriceOrders);
		...
		var _get = function(id, success, suppress) {
			User.get(function(user) {
				...
				currentOrder
					? (function() {
							_extend(currentOrder, user);
							_then(success, currentOrder);
						})()
					: $resource($451.api("order/:id"), { id: "@id" })
							.get({ id: id })
		...
		var _submit = function(order, success, error) {
			$resource($451.api("order"), {}, { submit: { method: "PUT" } })
				.submit(order)
				.$promise.then(
					function(o) {
		...
		var _approve = function(order, success, error) {
			$resource(
				$451.api("order/approve/:id"),
				{ id: order.ID },
				{
					approve: { method: "PUT", params: { comment: order.ApprovalComment } }
				}
			)
				.approve()
				.$promise.then(
		...
		var _decline = function(order, success, error) {
			$resource(
				$451.api("order/decline/:id"),
				{ id: order.ID },
				{
					decline: { method: "PUT", params: { comment: order.ApprovalComment } }
				}
			)
				.decline()
				.$promise.then(
		...
		var _deletelineitem = function(id, lineitemid, success, error) {
			...
			$resource(
				$451.api("order/:id/lineitem/:lineitemid"),
				{ id: id, lineitemid: lineitemid },
				{ lineitemdelete: { method: "DELETE" } }
			)
				.lineitemdelete()
				.$promise.then(
					function(o) {
		...
		var _calcdisc = function(order, acct) {
			...
			var discount = 0;
			if (
				acct.AccountType.MaxPercentageOfOrderTotal &&
				acct.AccountType.MaxPercentageOfOrderTotal != 100
			) {
		...
		var _listShipments = function(order, success) {
			$resource($451.api("order/:id/shipments"), { id: order.ID })
				.query()
