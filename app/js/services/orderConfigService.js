four51.app.factory("OrderConfig", [
	"Address",
	function(Address) {
		var user, order;
		...
		var setPaymentMethod = function(accounts) {
			...
			if (
				(user.Permissions.contains("SubmitForApproval") &&
					order.Approvals.length > 0) ||
				(order.Total === 0 && !user.Company.BillZeroPriceOrders)
			) {
				order.PaymentMethod = "Undetermined";
				return;
			}
			if (
				user.Permissions.contains("PayByBudgetAccount") &&
				accounts.length > 0
			) {
				order.PaymentMethod = "BudgetAccount";
				return;
			}
			if (
				user.Permissions.contains("PayByCreditCard") &&
				user.AvailableCreditCards.length > 0
			) {
		...
		var showOrderDetails = function() {
			return (
				user.Permissions.contains("EditPOID") ||
				user.Permissions.contains("Comments") ||
				(user.Permissions.contains("CostCenterPerOrder") &&
					!user.Permissions.contains("CostCenterPerLine") &&
					user.CostCenters.length > 0) ||
				(user.Permissions.contains("CostCenterPerOrder") &&
					user.Permissions.contains("FreeFormCostCenter")) ||
		...
		return {
			address: function(o, u) {
				...
				if (!_hasAddress()) setDefaultAddress();
				return this;
			},
			...
			paymentMethod: function(o, u, a) {
				// not used. set in the api now but leaving here for potential
				order = o;
				user = u;
				if (order.PaymentMethod == "Undetermined") {
					// might be legitimately this type, but can't be another unless already altered
					setPaymentMethod(a);
				}
				return this;
			},
