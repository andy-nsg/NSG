four51.app.controller("SpecFormCtrl", [
	"$scope",
	"$location",
	"$route",
	"$routeParams",
	...
	function(
		...
		function init() {
			$scope.variantErrors = [];
			var varID =
				$routeParams.variantInteropID == "new"
					? null
			...
			ProductDisplayService.getProductAndVariant(
				...
				function(data) {
					...
					if (varID) $scope.Variant = data.variant;
					else {
						$scope.Variant = {};
			...
			function saveVariant(
				variant,
				saveNew,
				hideErrorAlert /*for compatibility*/
			) {
				...
				Variant.save(variant, function(data) {
					if ($scope.isEditforApproval || $scope.EditingLineItem) {
						...
						$location.path(
							"/product/" + $scope.Product.InteropID + "/" + data.InteropID
						);
					}
				});
			}
