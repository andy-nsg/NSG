four51.app.controller("AddressInputCtrl", [
	"$scope",
	"$rootScope",
	"$location",
	"User",
	...
	function($scope, $rootScope, $location, User, Address, Resources) {
		$scope.save = function() {
			...
			Address.save(
				...
				function(ex) {
					if (ex.Code.is("ObjectExistsException")) $scope.objectExists = true;
				}
			);
		};
		...
		$scope.cancel = function() {
			$scope.return
				? $location.path($scope.return)
				: $rootScope.$broadcast("event:AddressCancel");
		};
		...
		$scope.hasStates = function() {
			return $scope.address != null
				? $scope.address.Country == "US" ||
						...
						$scope.address.Country == "NL"
				: false;
		};

		$scope.isPhoneRequired = function() {
			return (
				($scope.user.Permissions.contains("BillingAddressPhoneRequired") &&
					$scope.address.IsBilling) ||
				($scope.user.Permissions.contains("ShipAddressPhoneRequired") &&
					$scope.address.IsShipping) ||
				($scope.user.Permissions.contains("BillingAddressPhoneRequired") &&
					$scope.user.Permissions.contains("ShipAddressPhoneRequired"))
			);
		};
		...
		$scope.makeAddress = function(addressobj) {
			...
			addressobj.formatted_phone_number
				? ($scope.address.Phone = addressobj.formatted_phone_number)
				: "";
			angular.forEach(addressobj.address_components, function(component) {
				...
				component.types[0] == "administrative_area_level_1"
					? ($scope.address.State = component.short_name)
					: "";
				component.types[0] == "country"
					? ($scope.address.Country = component.short_name)
					: "";
				component.types[0] == "postal_code"
					? ($scope.address.Zip = component.short_name)
					: "";
			});
		};
	}
