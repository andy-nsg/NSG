four51.app.controller("Four51Ctrl", [
	"$scope",
	"$route",
	"$location",
	"$451",
		...
		if ($(window).width() < 960) {
			$(document)
				.on("focus", ':input:not("button")', function(e) {
					$(".navbar-fixed-bottom, .headroom.navbar-fixed-top").css(
						"position",
						"relative"
					);
				})
		...
		function init() {
			if (Security.isAuthenticated()) {
				User.get(function(user) {
					$scope.user = user;
					$scope.user.Culture.CurrencyPrefix = XLATService.getCurrentLanguage(
						user.CultureUI,
						user.Culture.Name
					)[1];
					$scope.user.Culture.DateFormat = XLATService.getCurrentLanguage(
						user.CultureUI,
						user.Culture.Name
					)[2];
