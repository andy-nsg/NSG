four51.app.controller("ErrorCtrl", [
	"$scope",
	function($scope) {
		$scope.open = function() {
			$scope.isError = true;
		...
		$scope.$on("exception", function(event, exception) {
			...
			try {
				...
				console.log(
					"An error occurred while handling an error. Consult the object written to the log. Keep in mind you must use the Error() if you are throwing an error in your JavaScript"
				);
				console.dir(event);
				console.dir(ex);
			}
