four51.app.controller("LineItemEditCtrl", [
	...
	function(
		...
		function init() {
			$scope.LineItem = {};
			$scope.LineItem =
				$scope.currentOrder.LineItems[$routeParams.lineItemIndex];
			if ($scope.LineItem.Variant && $scope.LineItem.Variant.InteropID) {
				ProductDisplayService.getProductAndVariant(
					$scope.LineItem.Product.InteropID,
					$scope.LineItem.Variant.InteropID,
					function(data) {
		...
		$scope.addToOrder = function() {
			...
			Order.save(
				...
				function(o) {
					...
					$location.path(
						"/cart" + ($scope.isEditforApproval ? "/" + o.ID : "")
					);
				},
				function(ex) {
					console.log(ex);
