	...
	function(AppConst) {
		function json_filter(input, options, op) {
			...
			angular.forEach(input, function(row) {
				if (row[options.Property] === undefined && !options instanceof Array)
					return;
				...
				if (options instanceof Array) {
					//does an OR on the list of conditions. could add to it to optionally have OR/AND
		...
		function arrayContainsValue(array, value) {
			if (angular.isArray(value)) {
				...
				return found;
			} else return array.indexOf(value) > -1;
		}
...
four51.apiName = function() {
	return four51.app.ApiAppName
		? four51.app.ApiAppName
