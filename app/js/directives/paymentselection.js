four51.app.directive("paymentselector", function() {
	var obj = {
		...
		controller: [
			...
			"SpendingAccount",
			"Order",
			function($scope, $rootScope, SavedCreditCard, SpendingAccount, Order) {
				...
				$scope.$watch("currentOrder.PaymentMethod", function(event) {
					if (event == "BudgetAccount" && $scope.SpendingAccounts) {
						if ($scope.SpendingAccounts.length == 1)
							$scope.currentOrder.BudgetAccountID =
								$scope.SpendingAccounts[0].ID;
						else {
							var count = 0,
								account;
					...
					$scope.cart_billing.$setValidity(
						"paymentMethod",
				...
				var budgetAccountCalculation = function(value) {
					...
					$scope.remainingOrderTotal = Order.calculatediscount(
						$scope.currentOrder,
						$scope.currentBudgetAccount
					);
				...
				function validatePaymentMethod(method) {
					...
					var validateAccount = function() {
						...
						angular.forEach($scope.SpendingAccounts, function(a) {
							if (
								$scope.currentOrder &&
								a.ID == $scope.currentOrder.BudgetAccountID
							)
								account = a;
						});
						if (account) {
							...
							if (account.Balance < $scope.currentOrder.Total) {
								...
								return paymentMethod == "BudgetAccount"
									? account.AccountType.AllowExceed
									: true;
							} else return true;
						}
						return paymentMethod != "BudgetAccount";
					...
					switch (method) {
						case "Undetermined":
							valid =
								$scope.user.Permissions.contains("SubmitForApproval") ||
								!$scope.currentOrder.BillingEnabled;
				...
				$rootScope.$on("event:SpendingAccountUpdate", function(
					event,
					accounts
				) {
				...
				$scope.deleteSavedCard = function(id) {
					if (
						confirm(
							"Are you sure you wish to delete this saved credit card? This cannot be undone"
						) == true
					) {
