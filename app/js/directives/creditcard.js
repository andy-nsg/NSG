		...
		controller: [
			...
			function($scope) {
				function validateType(type) {
					if (!type)
						// denotes a type we dont' even support.
						return true;
					if (type == "AmericanExpress")
						return $scope.user.Permissions.contains("PayByAmex");
					else return $scope.user.Permissions.contains("PayBy" + type);
				}
				 ...
				 name Mod 10 or Modulus 10. */
					...
					if (len == 0 && bResult)
						...
						if (len >= 14) {
							...
							else bResult = false; // This could NOT be a valid credit card number
						}
					}
					return bResult;
				}

				$scope.$watch("currentOrder.CreditCard.AccountNumber", function(
					ccnumber
				) {
					...
					if (ccnumber && len >= 4) {
						...
						$scope.creditCardIconUrl = cardType
							? "css/images/CreditCardIcons/" + cardType + ".png"
							: null;
