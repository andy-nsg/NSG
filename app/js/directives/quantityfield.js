			...
			link: function(scope) {
				...
				scope.validQuantityAddToOrder = function(value, lineItem) {
					...
					if (priceSchedule.MinQuantity > value) {
						scope.valid = false;
						scope.lineitem.qtyError =
							"must be equal or greater than " + priceSchedule.MinQuantity;
					}

					if (priceSchedule.MaxQuantity && priceSchedule.MaxQuantity < value) {
						scope.lineitem.qtyError =
							"must be equal or less than " + priceSchedule.MaxQuantity;
						scope.valid = false;
					}
					...
					if (product.IsVariantLevelInventory && !variant) {
						...
						var qtyAvail =
							(product.IsVariantLevelInventory
								? variant.QuantityAvailable
								: product.QuantityAvailable) + (lineItem.OriginalQuantity || 0);

						if (
							qtyAvail < value &&
							product.AllowExceedInventory == false &&
							priceSchedule.OrderType != "Replenishment"
						) {
							scope.lineitem.qtyError =
								"cannot exceed the Quantity Available of " + qtyAvail;
							scope.valid = false;
						}
					}
