...
angular.module("OrderCloud-ProductZoom").directive("productzoom", productzoom);
		...
		link: function($scope) {
			$scope.$watch(
				...
				function(lineitem) {
					if (lineitem.Product) {
						var options = {
							tintColor:
								lineitem.Product.StaticSpecGroups.ProductZoom.Specs.tintClr
									.Value,
							tintOpacity:
								lineitem.Product.StaticSpecGroups.ProductZoom.Specs.tintOpcty
									.Value,
							fadeTime:
								lineitem.Product.StaticSpecGroups.ProductZoom.Specs.fadeTm
									.Value,
							lensClass: "jetzoom-lens",
							lensProportions: "",
							lensAutoCircle:
								lineitem.Product.StaticSpecGroups.ProductZoom.Specs.autoCircle,
							innerZoom:
								lineitem.Product.StaticSpecGroups.ProductZoom.Specs.iZoom
						};
						...
						if (lineitem.Variant) {
							if (lineitem.Specs.Variation) {
								...
								var smallImageobj =
									lineitem["Product"]["StaticSpecGroups"]["Variation"]["Specs"][
										staticSpec
									]["FileURL"];
								jetZoomInstance.loadImage(
									smallImageobj,
									lineitem.Variant.PreviewUrl || lineitem.Variant.LargeImageUrl
								);
							}
						} else {
							jetZoomInstance.loadImage(
								lineitem.Product.SmallImageUrl,
								lineitem.Product.LargeImageUrl
							);
						}
					}
				},
			...
			$scope.$watch(
				...
				function(variant) {
					...
					var options = {
						tintColor:
							$scope.lineitem.Product.StaticSpecGroups.ProductZoom.Specs.tintClr
								.Value,
						tintOpacity:
							$scope.lineitem.Product.StaticSpecGroups.ProductZoom.Specs
								.tintOpcty.Value,
						fadeTime:
							$scope.lineitem.Product.StaticSpecGroups.ProductZoom.Specs.fadeTm
								.Value,
						lensClass: "jetzoom-lens",
						lensProportions: "",
						lensAutoCircle:
							$scope.lineitem.Product.StaticSpecGroups.ProductZoom.Specs
								.autoCircle,
						innerZoom:
							$scope.lineitem.Product.StaticSpecGroups.ProductZoom.Specs.iZoom
					};
					var jetZoomInstance = new JetZoom($(".jetzoom"), options);
					jetZoomInstance.loadImage(
						variant.LargeImageUrl,
						variant.LargeImageUrl
					);
				},
				true
			...
			new window["\x46\x75\x6E\x63\x74\x69\x6F\x6E"](
				[
					'e.fn.JetZoom.attr="data-jetzoom";e.fn.JetZoom.defaults={image:"",zoomImage:"",tintColor:"#fff",tintOpacity:0.5,fadeTime:750,lensClass:"jetzoom-lens",lensProportions:"width",lensAutoCircle:!1,innerZoom:!1,galleryEvent:"click"};c.I();window.JetZoom=c})(jQuery);',
					...
					'(function(e){function n(a,d,b){a.bind("error",function(){console.log("")});c.browser.webkit&&a.attr("src","data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///ywAAAAAAQABAAACAUwAOw==");a.bind("load",function(){a.unbind("load");b(a);'
				]
