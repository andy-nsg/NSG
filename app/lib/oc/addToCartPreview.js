...
angular
	.module("OrderCloud-AddToCartPreview")
...
function AddToCartPreviewCtrl($scope, $location, AddToCartPreview, $timeout) {
	...
	$scope.addListToCart = function(list) {
		...
		if (!$scope.qtyTotalError) {
			...
			AddToCartPreview.addListToCart(
				list,
	...
	$scope.verifyVariantQty = function(item) {
		//Variant Validation
	...
	$scope.verifyTotalQty = function(list) {
...
function AddToCartPreview(User, Order) {
	...
	var _getMinMaxTotalQty = function(lineitem) {
		if (
			lineitem.Product.StaticSpecGroups &&
			lineitem.Product.StaticSpecGroups.listPreview
		) {
			lineitem.Product.MinMaxSet = true;
			lineitem.Product.MinTotalQty = lineitem.Product.StaticSpecGroups
				.listPreview.Specs.MinQty
				? +lineitem.Product.StaticSpecGroups.listPreview.Specs.MinQty.Value
				: null;
			lineitem.Product.MaxTotalQty = lineitem.Product.StaticSpecGroups
				.listPreview.Specs.MaxQty
				? +lineitem.Product.StaticSpecGroups.listPreview.Specs.MaxQty.Value
				: null;
		}
	};
	...
	var _validateVariantQty = function(lineitem, success) {
		...
		var qtyAvailable = lineitem.Variant
			? lineitem.Variant.QuantityAvailable
			: lineitem.Product.QuantityAvailable;
		if (!+qty) {
			errors += "<p>- Please Enter Valid Quantity</p>";
		}
		if (qty < minQty && minQty != 1 && qty != "") {
			//checks if qty meets price schedule min qty
			errors +=
				"<p>- Quantity must be equal or greater than " +
				minQty +
				' for "' +
		...
		if (
			(qty > maxTotalQty && maxTotalQty) ||
			(qty > maxQty && maxQty != null)
		) {
			//checks if qty meets total order max (important) and price schedule max qty (redundant)
			errors +=
				"<p>- Quantity must be equal or less than " +
				(maxTotalQty > maxQty && maxQty != null ? maxQty : maxTotalQty) +
				' for "' +
				lineitem.Product.Name +
				'"</p>';
		}
		if (
			totalVariantQty > qtyAvailable &&
			qtyAvailable != false &&
			lineitem.Product.AllowExceedInventory == false
		) {
			//checks if qty of each variant exceeds inventory and if it is allowed (important)
			errors +=
				"<p>- Cannot exceed the Quantity Available of " +
				qtyAvailable +
				' for "' +
				(lineitem.Variant.Description || lineitem.Product.Name) +
				'"</p>';
		}
		if (totalVariantQty > maxQty && maxQty != null) {
			//checks if qty of each variant meets price schedule max qty (important)
			errors +=
				"<p>- Total Quantity for this variant cannot exceed " +
				maxQty +
				" (Current Total Qty: " +
				(totalVariantQty - qty) +
				")</p>";
	...
	var _validateTotalQty = function(list, success) {
		...
		qtySubmitError += !list[0].Variant
			? "You cannot use this module with a completely static product"
			: "";
		qtySubmitError +=
			!RestrictedQty && minQty && totalQty < minQty
				? "- Total quantity must be equal or greater than " +
					minQty +
					" for this order (Current Quantity: " +
					totalQty +
					")"
				: "";
		qtySubmitError +=
			!RestrictedQty && maxQty && totalQty > maxQty
				? (qtySubmitError +=
						"- Total quantity must be equal or less than " +
						maxQty +
						" for this order (Current Quantity: " +
						totalQty +
						")")
	...
	var _checkIfInList = function(item, list, success) {
		...
		angular.forEach(list, function(i) {
			if (item.Variant.InteropID == i.Variant.InteropID) {
				entryError =
					"<p>-Cannot Enter Same Size Twice (Change Quantity Below)</p>";
			}
		});
	...
	function priceBreak(item, quantity) {
		...
		for (var b = 0; b < item.PriceSchedule.PriceBreaks.length; b++) {
			if (
				b === item.PriceSchedule.PriceBreaks.length - 1 ||
				(quantity >= priceBreaks[b].Quantity &&
					quantity < priceBreaks[b + 1].Quantity)
			) {
				item.UnitPrice = item.Markup
					? item.Markup + item.PriceSchedule.PriceBreaks[b].Price
					: item.PriceSchedule.PriceBreaks[b].Price;
				break;
			}
		}
	...
	var _addListToCart = function(list, order, user, success, error) {
		...
		Order.clearshipping(tempOrder).save(
			tempOrder,
			function(o) {
