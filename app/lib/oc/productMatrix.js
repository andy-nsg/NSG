...
angular
	.module("OrderCloud-ProductMatrix")
...
function ProductMatrixCtrl(
	...
	function init() {
		...
		if ($scope.product.IsVBOSS) {
			...
			ProductMatrix.build(
				...
				$scope.currentOrder,
				lineItemEdit,
				function(matrix, specCount, spec1Name, spec2Name) {
	...
	$scope.qtyChanged = function() {
		$scope.qtyError = "";
		ProductMatrix.validateQuantity(
			$scope.comboVariants,
			$scope.product,
			function(message) {
		...
		ProductMatrix.backOrderValidate(
			$scope.comboVariants,
			$scope.product,
			function(data) {
	...
	$scope.addVariantsToOrder = function() {
		...
		if ($scope.qtyError) {
			...
			if (!$scope.lineItemIndex) {
				ProductMatrix.addToOrder($scope.comboVariants, $scope.product, function(
					lineItems
				) {
					$scope.addToOrderIndicator = true;
				...
				if ($scope.specCount == 1) {
					angular.forEach($scope.comboVariants, function(variant) {
						if (variant[0].Quantity) {
							$scope.currentOrder.LineItems[$scope.lineItemIndex].Quantity =
								variant[0].Quantity;
						}
					});
					saveOrder($scope.currentOrder);
...
function ProductMatrix($451, Variant) {
	function _then(fn, data, count, s1, s2) {
		if (angular.isFunction(fn)) fn(data, count, s1, s2);
	}

	var _getMinMaxTotalQty = function(product) {
		if (product.StaticSpecGroups && product.StaticSpecGroups.Matrix) {
			product.MinTotalQty =
				product.StaticSpecGroups.Matrix.Specs.MinQty &&
				product.StandardPriceSchedule.MinQuantity == 1
					? +product.StaticSpecGroups.Matrix.Specs.MinQty.Value
					: null;
			product.MaxTotalQty =
				product.StaticSpecGroups.Matrix.Specs.MaxQty &&
				!product.StandardPriceSchedule.MaxQuantity
					? +product.StaticSpecGroups.Matrix.Specs.MaxQty.Value
					: null;
		}
	...
	var _build = function(product, order, lineItemEdit, success) {
		...
		function getVariantData(p, params, group) {
			Variant.get(
				{ ProductInteropID: p.InteropID, SpecOptionIDs: params },
				function(variant) {
					variant.DisplayName = [];
					variant.Markup = params.Markup;
					variant.tempSpecs = {};
					variant.ListOrder = params.ListOrder;
					variant.Show = lineItemEdit
						? variant.ExternalID == lineItemEdit.Variant.ExternalID
						: true;
					if (variant.Show && lineItemEdit)
						variant.Quantity = lineItemEdit.Quantity;
					angular.forEach(product.Specs, function(spec) {
						angular.forEach(spec.Options, function(option) {
	...
	var _validateQty = function(matrix, product, success) {
			...
			angular.forEach(group, function(variant) {
				...
				if (variant.Quantity) {
					if (!$451.isPositiveInteger(qty)) {
						qtyError +=
							"<p>Please select a valid quantity for " +
							variant.DisplayName[0] +
							" " +
							(variant.DisplayName[1] ? variant.DisplayName[1] : "") +
							"</p>";
					} else {
				...
				if (qtyAvail < qty && product.AllowExceedInventory == false) {
					qtyError =
						"<p>Quantity cannot exceed the Quantity Available of " +
						qtyAvail +
						" for " +
						variant.DisplayName[0] +
						" " +
		...
		if (
			!product.RestrictedQuantity &&
			product.MinTotalQty &&
			totalQty < product.MinTotalQty
		) {
			qtyError +=
				"Total quantity must be equal or greater than " +
				product.MinTotalQty +
				" for " +
				(product.Name ? product.Name : product.ExternalID);
		}

		if (
			!product.RestrictedQuantity &&
			product.MaxTotalQty &&
			totalQty > product.MaxTotalQty
		) {
			qtyError +=
				...
				if (item.Quantity > 0) {
					...
					for (var spec in product.Specs) {
						...
						if (!liSpecs[spec].Value) {
							liSpecs[spec].Value = item.tempSpecs[spec]
								? item.tempSpecs[spec].Value
								: null;
						}
					}
					var li = {
...
function definesvariant() {
	return function(value) {
		...
		angular.forEach(value, function(s) {
			if (s.DefinesVariant == false || s.ControlType == "Text") output.push(s);
		});
		return output;
	};
}
