four51.app.factory("Report", [
	"$resource",
	"$q",
	"$451",
	"Error",
	function($resource, $q, $451, Error) {
		var cache = [],
			download = null;
		...
		var _get = function(id, success, error) {
			$resource($451.api("report/:id"), { id: "@id" })
				.get({ id: id })
				.$promise.then(
					function(data) {
						_then(success, data);
		...
		var _download = function(id, success, error, page, pagesize) {
			...
			if (
				typeof cache[(page - 1) * pagesize] == "object" &&
				typeof cache[page * pagesize - 1] == "object"
			) {
				download.Data = cache;
				_then(success, download, cache.length);
			} else {
				$resource($451.api("report/:id/download"), { id: "@id" })
					.get({ id: id, page: page, pagesize: pagesize })
					.$promise.then(
						function(data) {
