    ...
    var obj = {
      ...
      controller: [
        ...
        function($scope) {
          ...
          $scope.$watch("currentOrder.ShipAddressID", function(newValue) {
            ...
            if (newValue) {
              Address.get(newValue, function(add) {
                if (
                  $scope.user.Permissions.contains("EditShipToName") &&
                  !add.IsCustEditable
                ) {
                  angular.forEach($scope.currentOrder.LineItems, function(
                    item
                  ) {
          ...
          $scope.$watch("currentOrder.LineItems[0].ShipFirstName", function(
            newValue
          ) {
          ...
          $scope.$watch("currentOrder.LineItems[0].ShipLastName", function(
            newValue
          ) {
          ...
          var setShipping = function(shippers) {
            var lower48 = ['AL', 'AZ', 'AR', 'CA', 'CO', 'CT', 'DC', 'DE', 'FL', 'GA', 'ID', 'IL', 'IN', 'IA', 'KS', 'KY', 'LA', 'ME', 'MD', 'MA', 'MI', 'MN', 'MS', 'MO', 'MT', 'NE', 'NV', 'NH', 'NJ', 'NM', 'NY', 'NC', 'ND', 'OH', 'OK', 'OR', 'PA', 'RI', 'SC', 'SD', 'TN', 'TX', 'UT', 'VT', 'VA', 'WA', 'WV', 'WI', 'WY'];
            var usOrder = false;
            angular.forEach($scope.shipaddresses, function(address) {
              if (address.ID === $scope.currentOrder.ShipAddressID) {
                if (address.Country === "US" && lower48.indexOf(address.State) !== -1) {
                  usOrder = true;
                }
              }
            });
          ...
          $scope.updateShipper = function(li) {
            ...
            if (!li) {
              ...
              angular.forEach($scope.currentOrder.LineItems, function(item) {
                item.ShipperName = $scope.currentOrder.Shipper ?
                  $scope.currentOrder.Shipper.Name :
                  null;
                item.ShipperID = $scope.currentOrder.Shipper ?
                  $scope.currentOrder.Shipper.ID :
                  null;
              });
